...
void  chat_private(int socketfd, struct Msg *msg)
{
	...
	printf ("\t\t请输入要发送的对象名称: \n");
	fgets(msg->toname, SIZE, stdin);   //从标准输入读进字符串
	int len = strlen(msg->toname);
	msg->toname[len-1] = '\0';    //最后一位总是要以\0结尾
	printf ("\t\t请输入要发送的内容: \n");
	fgets(msg->msg, 1024, stdin);
	len = strlen(msg->msg);
	msg->msg[len-1] = '\0';    //最后一位总是要以\0结尾
	strcpy(msg->fromname, msg->name);
...
{
	printf("\n\t\t正在下载,请稍后...\n");
	int fd = open(msg->msg, O_RDWR | O_CREAT, 0777);    //不存在则创建 
	if(fd == -1)
	{
		perror("open");
		printf("文件下载失败了\n");
	...
	int ret = read(socketfd, buf, size);   //返回是一个长度 ，把socketfd的文件读入到buf指向的内存中
	if(ret == -1)
	{
		perror("read");
		return;
	}
	write(fd, buf, ret); 		//将buf指向的内存中的内容写到fd里面，完成文件“下载”
	close(fd);
	sleep(1);
	printf("\t\t文件下载完成\n");
}
